...
<body>
		...
		<div class="info-box">
			...
			<div class="my-box">
				...
				<div style="color:black; padding:20px; font-size:20px">
					...
					<p>학년 : ${member.memberGrade}</p>
					<p>학번 : ${member.memberID}</p>
					<p>학과 : ${member.memberMajor}</p>
				</div>
				<c:if test="${empty checkOK.checkOK}">
					<a href="/selectSubject">
						<button class="btn btn-primary" style="margin-left:53px; background-color:#113769;">수강신청하러가기</button>
					</a>
				</c:if>
				<c:if test="${not empty checkOK.checkOK}">
					<a href="/myInfo">
						<button class="btn btn-primary" style="margin-left:53px; background-color:#113769;">신청내역확인하기</button>
					</a>
				</c:if>
			</div>
		</div>
	...
	<!--사이드 메뉴바-->
	<jsp:include page='./includes/headerAfterLogin.jsp'/>
