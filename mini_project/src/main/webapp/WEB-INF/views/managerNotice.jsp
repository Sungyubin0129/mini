...
<body>
	...
	<div class="wrap-contents">
	        	...
	        	<c:forEach items="${noticeList}" var="noticeList">
	            	<tr>
	                    ...
	                    <td style="width:200px;">
	                    	<a class="move" id="<c:out value="${noticeList.noticeNumber}"/>" href='#'>
        ...
        <!-- 추가버튼 -->
        <button class="btn btn-primary" type="button" id="insertNotice" onclick="location.href='/noticeInsert'" style="position:relative; margin-top:20px; float:right">
        	작성하기
        </button>
                    
        <!-- 검색 영역 -->
        <div class="search_wrap">
        	<form id="searchForm" action="/managerNotice" method="get">
            	<div class="search_input">
                	<input type="text" name="keyword" value='<c:out value="${pageMaker.cri.keyword}"></c:out>' style="width:300px; height:30px;" placeholder="검색할 제목을 입력하세요">
                  	<input type="hidden" name="pageNum" value='<c:out value="${pageMaker.cri.pageNum }"></c:out>'>
                   	<input type="hidden" name="amount" value='${pageMaker.cri.amount}'>
                 	<button class="btn btn-primary" style="margin-left:20px;">검 색</button>
                </div>
            </form>
      	</div>  
	...
	$(".move").on("click", function(e){
		...
		moveForm.append("<input type='number' name='noticeNumber' hidden='hidden' value='" + parseInt($(this).attr('id')) + "'>");
		moveForm.attr("action", "/noticeDetailList");
		moveForm.submit();
